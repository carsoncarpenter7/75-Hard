...
function App() {
  const [state, setState] = useState<HabitTrackerState>(() => {
    ...
    return saved ? JSON.parse(saved) : { habits: [], progress: [] };
  });
  ...
  const startDate = habitLastCreatedAt ? new Date(habitLastCreatedAt) : get75DayStartDate();
  ...
  const handleEditHabit = (habitId: string, newName: string) => {
    setState((prev) => ({
      ...prev,
      habits: prev.habits.map((habit) => (habit.id === habitId ? { ...habit, name: newName } : habit)),
    }));
  ...
  const handleRemoveHabit = (habitId: string) => {
    ...
    setState((prev) => ({
      ...
      progress: prev.progress.map((day) => ({
        ...day,
        habits: Object.fromEntries(Object.entries(day.habits).filter(([id]) => id !== habitId)),
      })),
    }));
  };
  ...
  const handleToggleHabit = (habitId: string, date: string) => {
    ...
    setState((prev) => {
      const dayProgress = prev.progress.find((p) => p.date === date) || { date, habits: {} };
      const updatedProgress = prev.progress.filter((p) => p.date !== date);
      const updatedDayProgress = {
        ...dayProgress,
        habits: { ...dayProgress.habits, [habitId]: !dayProgress.habits[habitId] },
      };

      return { ...prev, progress: [...updatedProgress, updatedDayProgress] };
    });
  };

  const todayProgress = state.progress.find((p) => p.date === today) || { habits: {} };
  const completedHabits = state.habits.filter((h) => todayProgress.habits[h.id] === true).length;

  const totalProgress = calculateProgress(state.progress.filter((p) => p.date === today), state.habits.length);

  const handleResetChallenge = () => {
    const today = new Date().toISOString().split("T")[0];
    setChallengeStartDate(new Date());
    setHabitLastCreatedAt(today);
    setState({ habits: [], progress: [{ date: today, habits: {} }] });
  };
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex items-center gap-4">
                <button onClick={handlePreviousWeek} className="px-2 py-1 text-sm text-gray-600 hover:bg-gray-100 rounded">
                  ← Previous Week
                </button>
                <span className="text-sm text-gray-500">{Math.round(totalProgress)}% Complete</span>
                <button onClick={handleNextWeek} className="px-2 py-1 text-sm text-gray-600 hover:bg-gray-100 rounded">
                  Next Week →
                </button>
              </div>
            </div>
          ...
          <div className="mb-8">
            <ProgressCalendar
              ...
              habits={state.habits}
              startDate={startDate}
              days={filteredDays} // Pass only today and future dates
            />
          </div>
          ...
          {state.habits.length > 0 ? (
            ...
            <div className="mt-8 text-center text-gray-500">Add your first habit to get started!</div>
          )}
          <button
